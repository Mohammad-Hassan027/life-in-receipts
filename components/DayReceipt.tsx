...
import { DATA, RECEIPT_BY_DAY, chapterFor, fmtDate, fmtMoney, fmtInt } from "@/lib/data";

export default function DayReceipt({
  day, setDay,
}: { day: string | null; setDay: (d: string) => void }) {
  ...
  const list = useMemo(() => {
    ...
    return rs.filter(
      (r) => r.d.includes(s) || r.headline.toLowerCase().includes(s) ||
        r.items.some((i) => i.l.toLowerCase().includes(s) || i.sub.toLowerCase().includes(s))
    );
  }, [q]);
  ...
  const r = useMemo(() => {
    ...
    let best = DATA.receipts[0], bd = Infinity;
        ...
        <div className="space-y-0.5">
          {list.map((x) => (
            <button
              key={x.d}
              onClick={() => setDay(x.d)}
              className={`w-full text-left px-2 py-1.5 rounded text-[11.5px] flex justify-between gap-2 transition
                ${x.d === r.d ? "bg-ink/15 font-bold" : "hover:bg-ink/7"}`}
            >
              <span className="tabular-nums">{x.d}</span>
              <span className="truncate opacity-60 max-w-[110px]">{x.headline}</span>
            </button>
          ))}
          {list.length === 0 && <p className="text-[12px] opacity-50 italic px-2">Nothing matches.</p>}
        </div>
      </aside>

      <div className="flex justify-center">
        <div className="paper w-full max-w-[460px] px-7 py-8 text-[12.5px]">
          <div className="text-center">
            <div className="text-[10px] tracking-[0.35em] opacity-50">ONE DAY</div>
            <h2 className="mt-2 text-[24px] font-bold tabular-nums">{fmtDate(r.d)}</h2>
            {ch && (
          ...
          <div className="grid grid-cols-4 gap-2 text-center">
            {[
              ...
              <div key={k}>
                ...
                <div className="text-[9px] tracking-[0.2em] opacity-50 mt-0.5">{k}</div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-1.5">
            {r.items.map((it, i) => (
              <div key={i} className="flex items-baseline gap-2">
                <span className="tabular-nums opacity-45 text-[11px] w-10 shrink-0">{it.t}</span>
                <span
                  className="w-1.5 h-1.5 rounded-full shrink-0"
                  style={{ background: it.kind === "music" ? "#e8e3d3" : "#d98c3f" }}
                />
                <div className="min-w-0 flex-1">
                  <div className={`truncate ${it.flag === "SKIP" ? "line-through opacity-45" : ""}`}>
                    {it.l}
                  </div>
                  <div className="text-[10.5px] opacity-50 truncate">{it.sub}</div>
                </div>
