...
function totalLine(c: Chapter): { label: string; value: string } {
  ...
  if (c.skipRate > 0.5)
    return {
      ...
      value: `${fmtInt(Math.round(c.plays * c.skipRate))} TRACKS`,
    };
  if (c.nightShare > 0.28)
    return {
      label: "TOTAL NIGHTS AWAKE",
      value: `${fmtInt(c.nightPlays)} PLAYS`,
    };
  if (c.spanishPlays > 500)
    return {
      label: "TOTAL IN ANOTHER LANGUAGE",
      value: `${fmtInt(c.spanishPlays)} PLAYS`,
    };
  return {
    label: "TOTAL TIME SPENT",
    value: fmtHours(c.minutes).toUpperCase(),
  };
}
  ...
  return (
    <div
      ...
      className={`paper w-full max-w-[430px] px-7 py-8 text-[13px] leading-[1.75] transition-all duration-700
        ${seen ? "opacity-100 translate-y-0 receipt-settle" : "opacity-0 translate-y-10"}`}
      style={
        {
          transitionDelay: `${(i % 3) * 90}ms`,
          "--receipt-tilt": `${((i % 3) - 1) * 1.5}deg`,
    ...
    >
      <div className="text-center">
        ...
        <h2 className="mt-2 text-[22px] font-bold tracking-tight uppercase">
          {c.title}
        </h2>
      ...
      <p className="mt-5 text-[12.5px] italic leading-relaxed opacity-80">
        {c.kicker}
      </p>
      ...
      <div className="space-y-1">
        ...
        <Row
          l="Money moved"
          r={c.spend ? fmtMoney(c.spend) : "NO RECORD"}
          dim={!c.spend}
        />
      </div>
      ...
      {c.topTracks.length > 0 && (
        <>
          ...
          <div className="space-y-1">
            {c.topTracks.map((t2) => (
              <Row
                key={t2.track + t2.artist}
                l={`${t2.track} — ${t2.artist}`}
                r={`×${t2.n}`}
              />
            ))}
          </div>
    ...
    <div className="flex flex-col items-center gap-10 pb-32">
      <div className="paper w-full max-w-[430px] px-7 py-8 text-[13px]">
        <div className="text-center">
          ...
          <h1 className="mt-2 text-[26px] font-bold uppercase leading-none">
            A Life
          </h1>
